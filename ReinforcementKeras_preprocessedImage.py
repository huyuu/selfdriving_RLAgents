...
from PreprocessedImageEnvironmentClass import PreprocessedImageEnvironment
...
if __name__ == '__main__':
    ...
    env = PreprocessedImageEnvironment()
    ...
    image_inputs = keras.layers.Input(shape=env.observation_spec['image']['shape'], dtype=np.float, name='image')
    # image_inputs = layers.Input(shape=env.observation_spec['image']['shape'], dtype=np.float, name='image')
    image_layer = keras.layers.Conv2D(filters=64, kernel_size=4, strides=(2, 2), activation='relu', name='image_conv1')(image_inputs)
    image_layer = keras.layers.MaxPooling2D(pool_size=(4,4))(image_layer)
    image_layer = keras.layers.Conv2D(filters=32, kernel_size=3, strides=(1, 1), activation='relu', name='image_conv3')(image_layer)
    image_layer = keras.layers.MaxPooling2D(pool_size=(4,4))(image_layer)
    image_layer = keras.layers.Flatten(name='flattened')(image_layer)
    image_layer = keras.layers.Dense(256, activation='relu', name='image_dense1')(image_layer)
    image_layer = keras.layers.Dense(64, activation='relu', name='image_dense2')(image_layer)
    image_layer = keras.layers.Dense(1, activation='tanh', name='image_dense3')(image_layer)

    subPara_inputs = keras.layers.Input(shape=env.observation_spec['subPara']['shape'], dtype=np.float, name='subPara')

    common = keras.layers.concatenate([image_layer, subPara_inputs])
    common = keras.layers.Dense(64, activation="tanh", name='common_dense1')(common)
    common = keras.layers.Dense(64, activation="relu", name='common_dense2')(common)
    num_actions = env.action_spec['shape'][0]
    action = keras.layers.Dense(num_actions, activation="softmax", name='common_output')(common)


    # if exitst, load previous model
    model = None
    modelDirPath = './savedModels/ReinforcementKeras_preprocessedImage'
    ...
    if os.path.exists(modelPath):
        ...
        dlModelPath = '../DLAgents/continuousModel.h5'
        if os.path.exists(dlModelPath):
            dlModel = keras.models.load_model(dlModelPath)
            for layerIndex in [1, 2, 3, 4, 5, 6, 7, 8]:
                model.layers[layerIndex].set_weights(dlModel.layers[layerIndex].get_weights())
        # rlModelPath = './savedModels/ReinforcementKeras/model.h5'
        # if os.path.exists(rlModelPath):
        #     rlModel = keras.models.load_model(rlModelPath)
        #     for targetIndex, sourceIndex in zip([11, 12], [1, 2]):
        #         model.layers[targetIndex].set_weights(rlModel.layers[sourceIndex].get_weights())
    ...
    while running_reward < 3000:  # Run until solved
        ...
        with tf.GradientTape() as tape:
            while episode_count_in_single_descent <= episodes_amount_needed_for_one_decent:
                ...
                while not done:
                    ...
                    if done:
                        ...
                        episode_reward_tank.append(episode_reward)
            ...
            records.append([gradient_descent_count, np.mean(episode_reward_tank), running_reward, loss_value])
