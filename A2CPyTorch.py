...
class AtariA2C(nn.Module):
    ...
    def forward(self, x):
        _common = self.conv(x)
        return self.policy(_common), self.value(_common)
...
if __name__ == "__main__":
    ...
    make_env = lambda: ptan.common.wrappers.wrap_dqn(gym.make('FixPolicy-v0'))
    # envs = [make_env() for _ in range(NUM_ENVS)]
    envs = [gym.make('FixPolicy-v0') for _ in range(NUM_ENVS)]
    ...
    with common.RewardTracker(writer, stop_reward=2000) as tracker:
