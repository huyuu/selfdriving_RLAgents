...
from GapContinuousEnvironmentClass import GapContinuousEnvironment
...
register(
    id='GapContinuous-v0',
    entry_point='GapContinuousEnvironmentClass:GapContinuousEnvironment'
)
...
if __name__ == '__main__':
    ...
    common = layers.Dense(128, activation="relu", name='common1')(inputs)
    common = layers.Dense(128, activation="relu", name='common2')(common)
    mu = layers.Dense(1, activation='tanh', name='mu')(common)
    variance = layers.Dense(1, activation='softplus', name='variance')(common)
    ...
    model = None
    modelDirPath = './savedModels/ReinforcementKeras_continuous'
    ...
    if os.path.exists(modelPath):
        ...
        model = keras.Model(inputs=inputs, outputs=[mu, variance])
        ...
        with tf.GradientTape() as tape:
            while episode_count_in_single_descent <= episodes_amount_needed_for_one_decent:
                ...
                while not done:
                    ...
                    mu, var = model(state_old)

                    # Sample action from action probability distribution
                    action = np.clip(np.random.normal(np.squeeze(mu), np.sqrt(np.squeeze(var))), -1, 1)
                    # action = np.random.choice(num_actions, p=np.squeeze(action_probs))
                    # action_probs_history.append(tf.math.log(action_probs[0, action]))
                    action_probs_history.append(action)
                    ...
                    print(f"state: {state_old[0]}, action_prob: {np.squeeze(mu)}, {np.squeeze(var)}, action = {action}, reward = {reward}")
            ...
            history = zip(action_probs_history, mu_history, variance_history, discounted_rewards_history)
            actor_losses = []
            for action, mu, var, ret in history:
                ...
                log_prob = -(action - mu)**2/(2*var) - tf.math.log(tf.math.sqrt(2*np.pi*var))
